...
import type { ExpenseListItem } from '../../application/types/ExpenseListItem'

type ExpenseListTableProps = {
  items: ExpenseListItem[]
  isLoading: boolean
  onVoid?: (expenseId: string, version: number) => void
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-[var(--ui-radius)] border border-dashed border-[var(--ui-border)] bg-[var(--ui-surface-muted)] px-6 py-12 text-center">
        <p className="text-sm font-medium text-[var(--ui-text)]">No expenses yet</p>
        <p className="mt-1 text-sm text-[var(--ui-text-muted)]">
          Record your first operating expense with the button above.
        </p>
      </div>
    )
  }
        ...
        <tbody className="divide-y divide-[var(--ui-border)] bg-[var(--ui-surface)]">
          {items.map((item) => (
            <tr key={item.id} className="hover:bg-[var(--ui-surface-muted)]">
              <td className="px-4 py-3 tabular-nums">{formatDate(item.expenseDateIso)}</td>
              <td className="px-4 py-3">{item.categoryLabel}</td>
              <td className="px-4 py-3 font-medium">{item.payeeName}</td>
