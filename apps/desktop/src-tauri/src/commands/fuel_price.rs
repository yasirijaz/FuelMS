...
pub fn fuel_price_list_products(
    ...
    match db.with_business(|conn| FuelProductRepository::new(conn).list_active()) {
...
pub fn fuel_price_get_product_by_code(
    ...
    match db.with_business(|conn| FuelProductRepository::new(conn).find_by_code(&code)) {
...
pub fn fuel_price_get_product_by_id(
    ...
    match db.with_business(|conn| FuelProductRepository::new(conn).find_by_id(&product_id)) {
        Ok(dto) => CommandResultDto::ok(dto),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn fuel_price_get_record_by_id(
    ...
    match db.with_business(|conn| FuelPriceRepository::new(conn).find_by_id(&record_id)) {
        Ok(dto) => CommandResultDto::ok(dto),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn fuel_price_get_active_by_product(
    ...
    match db
        .with_business(|conn| FuelPriceRepository::new(conn).find_active_by_product(&product_id))
    {
        Ok(dto) => CommandResultDto::ok(dto),
        Err(e) => CommandResultDto {
            ok: false,
...
pub fn fuel_price_list_active(
    ...
    match db.with_business(|conn| FuelPriceRepository::new(conn).list_active()) {
        Ok(rows) => CommandResultDto::ok(rows),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn fuel_price_list_due_scheduled(
    ...
    match db.with_business(|conn| FuelPriceRepository::new(conn).list_due_scheduled(&as_of_iso)) {
        Ok(rows) => CommandResultDto::ok(rows),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn fuel_price_save_new(
    ...
    match db.with_business(|conn| FuelPriceRepository::new(conn).save_new(&record)) {
...
pub fn fuel_price_update_record(
    ...
    match db.with_business(|conn| FuelPriceRepository::new(conn).update(&record)) {
