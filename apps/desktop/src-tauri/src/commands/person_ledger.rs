...
pub fn person_ledger_list_balances(
    ...
    match db.with_business(|conn| PersonLedgerRepository::new(conn).list_balances(&query)) {
...
pub fn person_ledger_list_entries(
    ...
    match db.with_business(|conn| PersonLedgerRepository::new(conn).list_entries(&query)) {
        Ok(rows) => CommandResultDto::ok(rows),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn person_ledger_record_borrow(
    ...
    match db.with_business(|conn| PersonLedgerRepository::new(conn).record_borrow(&input)) {
...
pub fn person_ledger_record_repay_borrowed(
    ...
    match db.with_business(|conn| PersonLedgerRepository::new(conn).record_repay_borrowed(&input))
    {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
...
pub fn person_ledger_record_lend(
    ...
    match db.with_business(|conn| PersonLedgerRepository::new(conn).record_lend(&input)) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn person_ledger_record_collect_loan(
    ...
    match db.with_business(|conn| PersonLedgerRepository::new(conn).record_collect_loan(&input)) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
