...
pub fn inventory_product_summary(
    ...
    match db.with_connection(|conn| InventoryRepository::new(conn).product_summary()) {
...
pub fn inventory_list_batches(
    ...
    match db.with_connection(|conn| InventoryRepository::new(conn).list_batches(&query)) {
        Ok(rows) => CommandResultDto::ok(rows),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn inventory_list_movements(
    ...
    match db.with_connection(|conn| InventoryRepository::new(conn).list_movements(&query)) {
        Ok(rows) => CommandResultDto::ok(rows),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
