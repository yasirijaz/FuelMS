...
pub fn report_profit_loss(
    ...
    match db.with_business(|conn| ReportsRepository::new(conn).profit_loss(&query)) {
...
pub fn report_fuel_sales_summary(
    ...
    match db.with_business(|conn| ReportsRepository::new(conn).fuel_sales_summary(&query)) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn report_fuel_product_ledger(
    ...
    match db.with_business(|conn| ReportsRepository::new(conn).fuel_product_ledger(&query)) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn report_cash_position(db: State<'_, AppDatabase>) -> CommandResultDto<CashPositionReportDto> {
    match db.with_business(|conn| ReportsRepository::new(conn).cash_position()) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn report_person_ledger_summary(
    ...
    match db.with_business(|conn| ReportsRepository::new(conn).person_ledger_summary()) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn report_trial_balance(db: State<'_, AppDatabase>) -> CommandResultDto<TrialBalanceReportDto> {
    match db.with_business(|conn| ReportsRepository::new(conn).trial_balance()) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
