...
pub fn fuel_sale_list(
    ...
    match db.with_business(|conn| FuelSaleRepository::new(conn).list(&query)) {
...
pub fn fuel_sale_get_by_id(
    ...
    match db.with_business(|conn| FuelSaleRepository::new(conn).find_by_id(&sale_id)) {
...
pub fn fuel_sale_record(
    ...
    match db.with_business(|conn| FuelSaleRepository::new(conn).record(&input)) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn fuel_sale_post(
    ...
    match db.with_business(|conn| FuelSaleRepository::new(conn).post(&input)) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn fuel_sale_void(
    ...
    match db.with_business(|conn| FuelSaleRepository::new(conn).void_sale(&input)) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
