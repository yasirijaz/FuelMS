...
pub fn operating_income_list(
    ...
    match db.with_connection(|conn| OperatingIncomeRepository::new(conn).list(&query)) {
...
pub fn operating_income_get_by_id(
    ...
    match db.with_connection(|conn| OperatingIncomeRepository::new(conn).find_by_id(&income_id)) {
...
pub fn operating_income_record(
    ...
    match db.with_connection(|conn| OperatingIncomeRepository::new(conn).record(&input)) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
...
pub fn operating_income_void(
    ...
    match db.with_connection(|conn| OperatingIncomeRepository::new(conn).void(&input)) {
        Ok(row) => CommandResultDto::ok(row),
        Err(e) => CommandResultDto {
            ok: false,
            value: None,
